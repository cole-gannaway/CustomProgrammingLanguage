...
public class Lexer {
		...
		while (data != -1) {
			...
			if (!(Character.isLetter(ch)) && !(Character.isDigit(ch)) && (ch != '_')) {
				...
				if (buffer.equals("import")) {
					...
					return new Lexeme("NULL",buffer,lineNumber);
				}
				else if (buffer.equals("lambda")) {
					return new Lexeme("LAMBDA",buffer,lineNumber);
				}
				else if (buffer.equals("endl")) {
	...
	public static Lexeme lex() throws IOException {
		...
		if (ch == '{') {
			...
			return new Lexeme("AMPERSAND","&",lineNumber) ;
		}
		else if (ch == '|') {
			return new Lexeme("VERT_BAR","|",lineNumber) ;
		}
			/*Operators*/
		else if (ch == '+') {
	...
	public static void main(String[] args) throws IOException {
		...
		Lexeme ptr = lx.lex();
		while (!ptr.type.equals("EOF")){
			System.out.print(ptr.type + " ");
			ptr = lx.lex();
		}
